...
class Autoscaler(object):
    def __init__(self, functions: List[Function]):
        self.scalers: Dict[str: self._Scaler_] = {
    ...
    def poke(self, request: Request):
        ...
        if len(concurrencies) >= PANIC_WINDOW_SEC:
            ...
            log.fatal(f"No concurrecy measurements for {request.dest}")
    ...
    def evaluate(self):
        for func, tracker in sim.state.throttler.trackers.items():
            ...
            ready_pod = len(tracker.instances) if len(tracker.instances) != 0 else 1
        ...
        # TODO: update mode according to whether actual_cc >= 2*#replicas.
        return
...
class Throttler(object):
    ...
    def handle(self, request: Request):
        ...
        for instance in tracker.instances:
            sim.log.info(f"{instance.func} Has instances")
