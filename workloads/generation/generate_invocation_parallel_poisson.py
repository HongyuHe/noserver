...
def main():
    ...
    parser.add_argument('-j', '--filename', dest='json_file', action='store',
                      help='Name of the json file for the output', default='test_parallel_s2_m170_t1000')
    ...
    opts.json_file = 'test_parallel_s{}_m170_t1000'.format(opts.stages)
    
    generate_invocation_csv(timestamp_list, [opts.json_file]*num_inital_invocations, 
        [int(opts.invocations)]*num_inital_invocations, 
        'test_harvest_parallel_json{}_invoke{}_poisson{}_load{}'.format(opts.json_file, str(total_num_requests), opts.mean, loads))
