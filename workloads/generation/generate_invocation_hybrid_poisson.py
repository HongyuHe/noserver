...
def main():
    ...
    parser.add_argument('-d', '--depth', dest='depth', action='store',
                      help='Number of depth per DAG', default=4)     
    parser.add_argument('-w', '--width', dest='width', action='store',
                      help='Number of width per DAG', default=4)                    
    parser.add_argument('-j', '--filename', dest='json_file', action='store',
                      help='Name of the json file for the output', default='test_hybrid_s2_m170_t1000')
    ...
    num_stages = int(opts.width) * int(opts.depth)
    # assert not total_num_invocations % num_stages, f"{total_num_invocations=} not divisable by {num_stages=}"
    ...
    timestamp_list = generate_poisson_list(opts.mean * (num_stages), loads, num_inital_invocations)
    
    opts.json_file = 'test_hybrid_l{}_w{}_m170_t1000'.format(opts.depth, opts.width)
    
    generate_invocation_csv(timestamp_list, [opts.json_file]*num_inital_invocations, 
        [int(opts.invocations)]*num_inital_invocations, 
        'test_harvest_hybrid_json{}_invoke{}_poisson{}_load{}'.format(opts.json_file, str(total_num_requests), opts.mean, loads))
